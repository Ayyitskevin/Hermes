...
function reconciliationDisclosure(receipt: ImportHistoryPreview): string {
  const value = reconcileImportReceipt(receipt);
  const qualifiedLabel = `Reconcile receipt for ${receipt.sourceLabel}, ${receipt.accountLabel}, ${receipt.importedAtLabel}`;
...
function importReceiptHistoryCard(
  ...
  return `<article class="card import-history-row" data-import-receipt="${escapeHtml(receipt.receiptId)}" aria-labelledby="${headingId}">
    <div class="section-title"><div><p class="card-label">${state}</p><h3 id="${headingId}" data-import-receipt-heading="${escapeHtml(receipt.receiptId)}" tabindex="-1">${escapeHtml(receipt.sourceLabel)}<span class="sr-only"> · ${state.toLowerCase()} receipt</span></h3></div><strong>${countNoun(reconciliation.acceptedRows, "accepted row")}</strong></div>
    ...
    ${reconciliationDisclosure(receipt)}
    ${rollbackHistory}
    ${allowRollback && !receipt.rolledBack ? `<button class="text-button" type="button" data-rollback-receipt="${escapeHtml(receipt.receiptId)}">Roll back this import</button>` : ""}
  </article>`;
}
...
export function importReceiptHistorySection(snapshot: JournalWorkspaceSnapshot): string {
  ...
  return `<section aria-labelledby="import-history-title">
    ...
    <div class="journal-list">${snapshot.importHistory.map((receipt, index) => (
      importReceiptHistoryCard(receipt, index, allowRollback)
    )).join("")}</div>
  </section>`;
}
