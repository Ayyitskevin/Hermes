...
describe("SQLite journal export payload v2", () => {
  it("covers every v1-v5 table and column with deterministic string-safe rows", async () => {
    ...
    try {
      ...
      expect(first.tables.flatMap((table) => table.columns)).toHaveLength(314);
      ...
      expect(migrations.rows.map((row) => row[0])).toEqual(["1", "2", "3", "4", "5"]);
