...
/**
 * Environment-independent SHA-256 used to fingerprint migration source.
 *
 * Keeping this synchronous and free of Node APIs makes the same migration
 * contract usable in Vitest, WebView startup, and native Capacitor builds.
 */
export function sha256Hex(input: string): string {
  ...
  for (let offset = 0; offset < message.length; offset += 64) {
    for (let word = 0; word < 16; word += 1) {
      schedule[word] = messageView.getUint32(offset + word * 4);
    }
    for (let word = 16; word < 64; word += 1) {
      const previous15 = schedule[word - 15] ?? 0;
      const previous2 = schedule[word - 2] ?? 0;
    ...
    for (let round = 0; round < 64; round += 1) {
      ...
      const temporary1 = (h + sum1 + choice + (SHA_256_ROUND_CONSTANTS[round] ?? 0) + (schedule[round] ?? 0)) >>> 0;
