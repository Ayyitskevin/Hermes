  ...
  return page.evaluate(() => {
    ...
    return {
      ...
      symbols: {
        ...
        groups: text("[data-symbol-breakdown] .symbol-breakdown-groups"),
        evidence: attributes(
      ...
      openingWeekday: {
        ...
        groups: text(
          "[data-opening-weekday-mix] .opening-weekday-mix-groups",
        ),
        evidence: attributes(
...
test("exact card facets compose with search and scope without changing totals or reports", async ({ page }) => {
  ...
  await expect(page.locator("[data-opening-weekday-mix]"))
