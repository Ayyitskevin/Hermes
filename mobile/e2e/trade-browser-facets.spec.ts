...
test("exact card facets compose with search and scope without changing totals or reports", async ({ page }) => {
  ...
  await expect(mistake).toHaveValue("");
  await expect(emotion).toHaveValue("");
  ...
  await expect(page.getByRole("combobox", { name: "Mistake" })).toHaveValue("");
  await expect(count).toHaveText("Showing 8 trades");
...
test("seven exact card facets reflow at 320px and 200% text with touch-size controls", async ({ page }) => {
